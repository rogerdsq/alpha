...
while connected_to_net():
    ...
    for t in tickers:
        ...
        try:
            ...
            pat = re.compile('ref_\d+_op">(\d*,*\d+\.\d+)</span>')
            rlt = pat.search(html)
            if rlt:
                row_open.append(rlt.group(1).rjust(13))
            pat = re.compile('ref_\d+_lo">(\d*,*\d+\.\d+)</span>')
            rlt = pat.search(html)
            if rlt:
                row_low.append(rlt.group(1).rjust(13))
            pat = re.compile('ref_\d+_hi">(\d*,*\d+\.\d+)</span>')
            rlt = pat.search(html)
            if rlt:
                row_high.append(rlt.group(1).rjust(13))
